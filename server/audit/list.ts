...
async function handler(req: RequestLike, res: ResponseLike) {
  ...
  const response = await fetch(
    `${url}/rest/v1/cltx_kv_store?select=key,value,expires_at&key=like.audit%3A*&order=expires_at.desc&limit=200`,
  ...
  const entries = rows.map(row => {
    try {
      return JSON.parse(row.value) as {
        at: string; actor: string; action: string; licenseId?: string; tier?: string;
      };
    } catch { return null; }
